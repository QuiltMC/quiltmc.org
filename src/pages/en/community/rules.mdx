...
All nicknames must adhere to the rules above. Additionally, nicknames have to adhere to the following rules:
1. Nicknames must be easy to read, especially by a screen-reader (ex: no Zalgo or otherwise hard to read non-ascii characters, with the exception of non-Latin names)
2. Nicknames cannot be intentionally confusing or misleading (ex: impersonation is stricly forbidden, as well as using similar looking nicknames to be intentionally confusing)
3. Nicknames must have some readable characters (ex: no "empty" nicknames)

We also reserve the right to change or reset nicknames that are infringing those rules, or designed to place you higher in the member list in community spaces that have one, which is known as "hoisting".
...
All user statuses must adhere to the rules above. In particular, we don't want to see objectionable or <abbr title="Not Safe For Work">NSFW</abbr> emoji, the advertising of sketchy or <abbr title="Not Safe For Work">NSFW</abbr> websites, or hateful sentiment.
