...
Using [Visual Studio Code](https://code.visualstudio.com) or [VSCodium](https://vscodium.com) is recommended. When opening the site with VSCode, you should see a popup in the bottom right asking if you want to install recommended extensions. If you don't see it, you can also type `@recommended` into the extensions serachbar:
...
After installing, run `pnpm i` and then `pnpm dev`. Now go to `localhost:3000` in your browser, and you can see the site. It will update automatically if you make any changes.
...
A frontmatter block must be placed at the top of every document or page that's written for the site. It takes the form of a block of YAML surrounded by triple dashes (`---`), and should contain _at least_ a `title` and `layout` variable.
...
Pages that don't contain a `layout` variable in the frontmatter will be outputted as plain HTML (no sidebar, no styling, nothing). Below are some frontmatter variables:
...
Author information can be specified using the `authors` property in the frontmatter:
...
To use any components that require importing, you'll need to be in a `.mdx` file instead of a `.md` file. There's no difference between the markdown in these two files types, so you can just rename an existing file.

#### Messages

`Message` -- this allows you to create an Message, as shown below. Messages are essentially boxes that draw attention to a specific block of text.
...
For blog posts, you need to include a `date` field in the forntmatter, which should be in a standard ISO format (For example, `2022-05-07 4:30:00 -00:00`). This is required for all blog posts, as the site relies on it for post ordering and to display metadata.
